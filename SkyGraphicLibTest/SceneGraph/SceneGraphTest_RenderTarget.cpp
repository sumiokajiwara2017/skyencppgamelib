...
//--------3DScreenのセットアップ--------▽
	...
	g_spModel_1->LeafList().DeleteAll();
	g_spModel_1->AttachLeaf( 0 , graphic::CTextureMappingShaderObject::Create() );
	g_spModel_2 = graphic::CModelUtil::CreateFromWavefrontOBJ( CHSC( TEST1_OBJ_FILE_NAME_1 ) );
	g_spModel_2->LeafList().DeleteAll();
	g_spModel_2->AttachLeaf( 0 , graphic::CTextureMappingShaderObject::Create() );
	...
	spCRenderTarget->SetUpTexture( math::CBasicSize( _WIN_W / 4 , _WIN_H / 4 ) );
	spCRenderTarget->SetClearColor( graphic::CColor_DeepBlue );

	graphic::CViewPortPtr spViewPort = CViewPortCreate_();
	spViewPort->SetRect(math::CBasicRect( 0 , 0 , _WIN_W / 4 , _WIN_H / 4 ) );
	spCRenderTarget->AttachChild( spViewPort );
	...
	spViewPort->AttachChild( g_spModel_2 );

	//メッシュにテクスチャを設定する
	g_spModel_1->GetMesh()->SetDiffuseMap( spTexture );

	//モデルをシーングラフに登録
	g_spModel_1->SetDepthBufferClear( skyTrue );
...
//--------3DScreenのセットアップ--------△
	...
	GAME_LOOP_START
	{
		...
		//モデルの回転
		g_spModel_1->GetEntity3D()->m_Entity3D.Rot.Y = CMathUtil_::RadToDeg( math::util::KPI * ( ( u32 )CTimeUtil_::GetNowTime() % 3000 ) / 1500.0f );
		g_spModel_1->GetEntity3D()->m_Entity3D.Rot.X = CMathUtil_::RadToDeg( math::util::KPI * ( ( u32 )CTimeUtil_::GetNowTime() % 1500 ) / 750.0f );
		g_spModel_2->GetEntity3D()->m_Entity3D.Rot.Y = CMathUtil_::RadToDeg( math::util::KPI * ( ( u32 )CTimeUtil_::GetNowTime() % 3000 ) / 1500.0f );
		g_spModel_2->GetEntity3D()->m_Entity3D.Rot.X = CMathUtil_::RadToDeg( math::util::KPI * ( ( u32 )CTimeUtil_::GetNowTime() % 1500 ) / 750.0f );
