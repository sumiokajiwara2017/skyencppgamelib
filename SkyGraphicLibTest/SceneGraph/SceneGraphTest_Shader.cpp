...
//--------3DScreenのセットアップ--------▽
	...
	g_spModel->DetachShaderObjectAll(); //わざとデフォルトのシェーダーを消す。

	//シェーダーの作成
	graphic::CBasicShaderObjectPtr spShader_1 = graphic::CBasicShaderObject::Create();
	graphic::CBasicShaderObjectPtr spShader_2 = spShader_1->vCloneNode();

	//ルートは空のノードにする
	g_spScreen->GetScene()->m_spSceneGraph = CSceneGraphNodeCreate_();

	//モデルを子として登録
	g_spScreen->GetScene()->m_spSceneGraph->AttachChild( spShader_1 );
	spShader_1->AttachChild( g_spModel );
	...
	spViewPort->AttachChild( spShader_2 ); 
	spShader_2->AttachChild( g_spModel );
