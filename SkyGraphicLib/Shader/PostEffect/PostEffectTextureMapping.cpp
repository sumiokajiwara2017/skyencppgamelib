...
SkyImplementRTTI( sky::lib::graphic::CPostEffectTextureMapping , sky::lib::graphic::CPostEffectObject );
...
//頂点シェーダー入力値構造体
struct CPostEffectTextureMappingConst
{
	//頂点変換行列
...
CShaderInfoPtr GetShaderInfo( u32 shaderId )
{
	...
	if ( s_pShaderHash->Get( shaderId , spShaderInfo ) == skyFalse )
	{
		...
		ConstDesc.ByteWidth			= sizeof( CPostEffectTextureMappingConst );
...
void CPostEffectTextureMapping::vRendering( u32 passNo , const CTexturePtr &spRenderTargetTexture , const CSamplerData &samplerData , const CMeshContenaPtr &spMesh , const math::CMatrix4x3 &cameraMatrix )
{
	SKY_UNUSED_ARG( passNo );
	...
	//定数バッファ受け渡し用の変数
	CPostEffectTextureMappingConst shaderConst;
...
CShaderInfoPtr GetShaderInfo( u32 shaderId )
{
	...
	if ( s_pShaderHash->Get( shaderId , spShaderInfo ) == skyFalse )
	{
		...
		sky::lib::graphic::CGLSL::CreateVertexShaderAndFragmentShader(
			hm_ProgramObject0 ,
			_T( "data/lib/shader/glsl/TextureMappingPostEffect.vert" ), 
			_T( "data/lib/shader/glsl/TextureMappingPostEffect.frag" ) );
...
CShaderInfoPtr GetShaderInfo( u32 shaderId )
{
	...
	if ( s_pShaderHash->Get( shaderId , spShaderInfo ) == skyFalse )
	{
		...
		CCg::CreateVertexFragmentCombineShaderFromFile( spShaderInfo->m_Pass1.m_ProgramObject , 
		                                            "data/lib/shader/cg/TextureMappingPostEffect.vert" , "mainVS" ,
		                                            "data/lib/shader/cg/TextureMappingPostEffect.frag" , "mainPS"  );
