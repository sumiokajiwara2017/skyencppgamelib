...
SkyImplementRTTI( sky::lib::graphic::CPointLightShaderObject       , sky::lib::graphic::CShaderObject );
...
static const math::CBasicVector3 POINT_LIGHT_OBJ_DEFAULT_LIGHT_POS = math::CBasicVector3( 5.0f , 5.0f , -5.0f );
static const f32                 POINT_LIGHT_OBJ_DEFAULT_LIGHT_POWER = 2.0f;

#define GETSET_BIN( isWrite ) \
	stream->vWriteReadStream( ( u8 * )&m_LightPosition.X , sizeof( d64 ) , isWrite ); \
	stream->vWriteReadStream( ( u8 * )&m_LightPosition.Y , sizeof( d64 ) , isWrite ); \
	stream->vWriteReadStream( ( u8 * )&m_LightPosition.Z , sizeof( d64 ) , isWrite ); \
	stream->vWriteReadStream( ( u8 * )&m_LightPower , sizeof( f32 ) , isWrite );

skyBool CPointLightShaderObject::SaveBinary( const stream::IStreamPtr& stream )
{
...
skyBool CPointLightShaderObject::LoadBinary( const stream::IStreamPtr& stream )
{
...
const serialize::CSerializeInterfacePtr& CPointLightShaderObject::Copy( const serialize::CSerializeInterfacePtr& spContainer )
{
	this->CSceneGraphNode::Copy( spContainer );
	( ( CPointLightShaderObjectPtr )spContainer )->m_LightPosition = m_LightPosition;
	( ( CPointLightShaderObjectPtr )spContainer )->m_LightPower = m_LightPower;
	return spContainer;
}

const serialize::CSerializeInterfacePtr& CPointLightShaderObject::CopySharing( const serialize::CSerializeInterfacePtr& spContainer )
{
	return Copy( spContainer );
}

serialize::CSerializeInterfacePtr CPointLightShaderObject::Clone()
{
	return Copy( CPointLightShaderObjectPtr( NEW__( CPointLightShaderObject , CPointLightShaderObject() ) ) );
}

serialize::CSerializeInterfacePtr CPointLightShaderObject::CloneSharing()
{
	return Clone();
}

CPointLightShaderObject::CPointLightShaderObject() : m_LightPosition( POINT_LIGHT_OBJ_DEFAULT_LIGHT_POS ) , m_LightPower( POINT_LIGHT_OBJ_DEFAULT_LIGHT_POWER )
{
	m_PassNum = 1;
}


void CPointLightShaderObject::vDraw()
{
	//シェーダーIDの構築（PointLightShaderObjectではライトの状態に応じてシェーダーを作る）
	u32 shaderId = 0;
...
static CShaderInfoPtr GetShaderInfo( u32 shaderId )
{
	...
	if ( s_pShaderHash->Get( shaderId , spShaderInfo ) == skyFalse )
	{
		...
		// 頂点シェーダ作成
		HRESULT hr = D3DX11CompileFromFile( _T( "data/lib/shader/hlsl/PointLightShader.vert" ) , NULL , NULL , "mainVS" , CDevice_->GetMember()->GetVSVersion() , SHADER_COMPILE_OPTION , 0 , NULL , &pShader , &pError , NULL );
		if ( FAILED( hr ) )
		{
		...
		ConstDesc.ByteWidth			  = sizeof( PointLightShaderConst );
		...
		// ピクセルシェーダ作成
		hr = D3DX11CompileFromFile( _T( "data/lib/shader/hlsl/PointLightShader.frag" ) , NULL , NULL , "mainPS" , CDevice_->GetMember()->GetPSVersion() , SHADER_COMPILE_OPTION , 0 , NULL , &pShader , &pError , NULL );
		if ( FAILED( hr ) )
		{
			D3DUtil::PrintD3D11Result( hr );
			OutputDebugStringA( (const char*)pError->GetBufferPointer() ); pError->Release();
...
void CPointLightShaderObject::Pass1Draw()
{
	//定数バッファ受け渡し用の変数
	PointLightShaderConst shaderConst;
	...
	// 点光源座標
	D3DUtil::ConvertVector3ToVector3( m_LightPosition * m_spCamera->GetViewMatrix() , shaderConst.LightPos );
	shaderConst.LightPower = m_LightPower;
...
static CShaderInfoPtr GetShaderInfo( u32 shaderId )
{
	...
	if ( s_pShaderHash->Get( shaderId , spShaderInfo ) == skyFalse )
	{
		...
		sky::lib::graphic::CGLSL::CreateVertexShaderAndFragmentShader(
			hm_ProgramObject0 ,
			_T( "data/lib/shader/glsl/PointLightShader.vert" ), 
			_T( "data/lib/shader/glsl/PointLightShader.frag" ) );
...
void CPointLightShaderObject::Pass1Draw()
{
	...
	// 点光源ベクトル計算設定
	math::CBasicVector3 lightPos = m_LightPosition * m_spCamera->GetViewMatrix();
	lightPos.Copyf32Array( g_GlobalParam.Light );
	g_GlobalParam.LightPower = m_LightPower;
	...
	FOREACH( CSceneGraphNodePtrList , itMat , pMaterialContena->GetMaterialDataArray() )
	{
		...
		//マテリアル情報をシェーダーに引き継ぎ
		spMatDrawData->GetMaterial()->Diffuse.Copyf32Array4RGBA( Diffuse );
...
static CShaderInfoPtr GetShaderInfo( u32 shaderId )
{
	...
	if ( s_pShaderHash->Get( shaderId , spShaderInfo ) == skyFalse )
	{
		...
		CCg::CreateVertexFragmentCombineShaderFromFile( spShaderInfo->m_Pass1.m_ProgramObject , 
		                                            "data/lib/shader/cg/PointLightShader.vert" , "mainVS" ,
		                                            "data/lib/shader/cg/PointLightShader.frag" , "mainPS"  );
        ...
        spShaderInfo->m_Pass1.m_DiffuseLocation = cgGetNamedParameter ( spShaderInfo->m_Pass1.m_ProgramObject.Program , "uniforms.Diffuse" );
     	CCg::CheckForCgError( "パラメーターの設定に失敗しました。" );
        spShaderInfo->m_Pass1.m_FreeLocation0 = cgGetNamedParameter ( spShaderInfo->m_Pass1.m_ProgramObject.Program , "uniforms.Light" );
     	CCg::CheckForCgError( "パラメーターの設定に失敗しました。" );
        spShaderInfo->m_Pass1.m_FreeLocation1 = cgGetNamedParameter ( spShaderInfo->m_Pass1.m_ProgramObject.Program , "uniforms.LightPower" );
     	CCg::CheckForCgError( "パラメーターの設定に失敗しました。" );
...
void CPointLightShaderObject::Pass1Draw()
{
	...
	// 点光源ベクトル計算設定
	math::CBasicVector3 lightPos = m_LightPosition * m_spCamera->GetViewMatrix();
	Light[ 0 ] = ( GLfloat )lightPos.X;
	Light[ 1 ] = ( GLfloat )lightPos.Y;
	Light[ 2 ] = ( GLfloat )lightPos.Z;
    cgSetParameter3fv( m_spShaderInfo->m_Pass1.m_FreeLocation0 , Light );

	// 光の力を渡す
	LightPower = m_LightPower;
    cgSetParameter1fv( m_spShaderInfo->m_Pass1.m_FreeLocation1 , &LightPower );
