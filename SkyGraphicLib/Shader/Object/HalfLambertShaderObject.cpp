...
SkyImplementRTTI( sky::lib::graphic::CHalfLambertShaderObject       , sky::lib::graphic::CShaderObject );
...
skyBool CHalfLambertShaderObject::SaveBinary( const stream::IStreamPtr& stream )
{
	SKY_UNDER_CONSTRUCTION( stream );
	return skyFalse;
}

skyBool CHalfLambertShaderObject::LoadBinary( const stream::IStreamPtr& stream )
{
	SKY_UNDER_CONSTRUCTION( stream );
	return skyFalse;
}

const serialize::CSerializeInterfacePtr& CHalfLambertShaderObject::Copy( const serialize::CSerializeInterfacePtr& spContainer )
{
	return spContainer;
}

const serialize::CSerializeInterfacePtr& CHalfLambertShaderObject::CopySharing( const serialize::CSerializeInterfacePtr& spContainer )
{
	return Copy( spContainer );
}

serialize::CSerializeInterfacePtr CHalfLambertShaderObject::Clone()
{
	return Copy( CHalfLambertShaderObjectPtr( NEW__( CHalfLambertShaderObject , CHalfLambertShaderObject() ) ) );
}

serialize::CSerializeInterfacePtr CHalfLambertShaderObject::CloneSharing()
{
	return Clone();
}

CHalfLambertShaderObject::CHalfLambertShaderObject()
{
	m_PassNum = 1;
}

void CHalfLambertShaderObject::vDraw()
{
...
void CHalfLambertShaderObject::Pass1Draw()
{
}
...
static CShaderInfoPtr GetShaderInfo( u32 shaderId )
{
	...
	if ( s_pShaderHash->Get( shaderId , spShaderInfo ) == skyFalse )
	{
		...
		sky::lib::graphic::CGLSL::CreateVertexShaderAndFragmentShader(
			hm_ProgramObject0 ,
			_T( "data/lib/shader/glsl/HalfLambertShader.vert" ), 
			_T( "data/lib/shader/glsl/HalfLambertShader.frag" ) );
