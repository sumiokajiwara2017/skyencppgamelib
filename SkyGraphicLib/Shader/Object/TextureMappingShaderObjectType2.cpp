...
SkyImplementRTTI( sky::lib::graphic::CTextureMappingShaderType2Object       , sky::lib::graphic::CShaderObject );
...
const serialize::CSerializeInterfacePtr& CTextureMappingShaderType2Object::Copy( const serialize::CSerializeInterfacePtr& spContainer )
{
	this->CSceneGraphNode::Copy( spContainer );
	return spContainer;
}

const serialize::CSerializeInterfacePtr& CTextureMappingShaderType2Object::CopySharing( const serialize::CSerializeInterfacePtr& spContainer )
{
	return Copy( spContainer );
}

serialize::CSerializeInterfacePtr CTextureMappingShaderType2Object::Clone()
{
	return Copy( CTextureMappingShaderType2ObjectPtr( NEW__( CTextureMappingShaderType2Object , CTextureMappingShaderType2Object() ) ) );
}

serialize::CSerializeInterfacePtr CTextureMappingShaderType2Object::CloneSharing()
{
	return Clone();
}

CTextureMappingShaderType2Object::CTextureMappingShaderType2Object()
{
	m_PassNum = 1;
}

void CTextureMappingShaderType2Object::vDraw()
{
...
//頂点シェーダー入力値構造体
struct TextureMappingShaderType2Const
{
	//頂点変換行列
...
static CShaderInfoPtr GetShaderInfo( u32 shaderId )
{
	...
	if ( s_pShaderHash->Get( shaderId , spShaderInfo ) == skyFalse )
	{
		...
		// 頂点シェーダ作成
		HRESULT hr = D3DX11CompileFromFile( _T( "data/lib/shader/hlsl/TextureMappingType2.vert" ) , NULL , NULL , "mainVS" , CDevice_->GetMember()->GetVSVersion() , SHADER_COMPILE_OPTION , 0 , NULL , &pShader , &pError , NULL );
		if ( FAILED( hr ) )
		{
		...
		ConstDesc.ByteWidth			= sizeof( TextureMappingShaderType2Const );
		...
		// ピクセルシェーダ作成
		hr = D3DX11CompileFromFile( _T( "data/lib/shader/hlsl/TextureMappingType2.frag" ) , NULL , NULL , "mainPS" , CDevice_->GetMember()->GetPSVersion() , SHADER_COMPILE_OPTION , 0 , NULL , &pShader , &pError , NULL );
		if ( FAILED( hr ) )
		{
			OutputDebugStringA( (const char*)pError->GetBufferPointer() ); pError->Release();
			D3DUtil::PrintD3D11Result( hr );
...
void CTextureMappingShaderType2Object::Pass1Draw()
{
	//定数バッファ受け渡し用の変数
	TextureMappingShaderType2Const shaderConst;
...
static CShaderInfoPtr GetShaderInfo( u32 shaderId )
{
	...
	if ( s_pShaderHash->Get( shaderId , spShaderInfo ) == skyFalse )
	{
		...
		sky::lib::graphic::CGLSL::CreateVertexShaderAndFragmentShader(
			hm_ProgramObject0 ,
			_T( "data/lib/shader/glsl/TextureMappingType2.vert" ), 
			_T( "data/lib/shader/glsl/TextureMappingType2.frag" ) );
...
static CShaderInfoPtr GetShaderInfo( u32 shaderId )
{
	...
	if ( s_pShaderHash->Get( shaderId , spShaderInfo ) == skyFalse )
	{
		...
		CCg::CreateVertexFragmentCombineShaderFromFile( spShaderInfo->m_Pass1.m_ProgramObject , 
		                                            "data/lib/shader/cg/TextureMappingType2.vert" , "mainVS" ,
		                                            "data/lib/shader/cg/TextureMappingType2.frag" , "mainPS"  );
