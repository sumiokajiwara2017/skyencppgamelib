...
SkyImplementRTTI( sky::lib::graphic::CBlinnPhongShaderObject       , sky::lib::graphic::CShaderObject );
...
skyBool CBlinnPhongShaderObject::SaveBinary( const stream::IStreamPtr& stream )
{
	SKY_UNDER_CONSTRUCTION( stream );
	return skyFalse;
}

skyBool CBlinnPhongShaderObject::LoadBinary( const stream::IStreamPtr& stream )
{
	SKY_UNDER_CONSTRUCTION( stream );
	return skyFalse;
}

const serialize::CSerializeInterfacePtr& CBlinnPhongShaderObject::Copy( const serialize::CSerializeInterfacePtr& spContainer )
{
	return spContainer;
}

const serialize::CSerializeInterfacePtr& CBlinnPhongShaderObject::CopySharing( const serialize::CSerializeInterfacePtr& spContainer )
{
	return Copy( spContainer );
}

serialize::CSerializeInterfacePtr CBlinnPhongShaderObject::Clone()
{
	return Copy( CBlinnPhongShaderObjectPtr( NEW__( CBlinnPhongShaderObject , CBlinnPhongShaderObject() ) ) );
}

serialize::CSerializeInterfacePtr CBlinnPhongShaderObject::CloneSharing()
{
	return Clone();
}

CBlinnPhongShaderObject::CBlinnPhongShaderObject()
{
	m_PassNum = 1;
}

void CBlinnPhongShaderObject::vDraw()
{
...
void CBlinnPhongShaderObject::Pass1Draw()
{
}
...
static CShaderInfoPtr GetShaderInfo( u32 shaderId )
{
	...
	if ( s_pShaderHash->Get( shaderId , spShaderInfo ) == skyFalse )
	{
		...
		sky::lib::graphic::CGLSL::CreateVertexShaderAndFragmentShader(
			hm_ProgramObject0 ,
			_T( "data/lib/shader/glsl/BlinnPhongShader.vert" ), 
			_T( "data/lib/shader/glsl/BlinnPhongShader.frag" ) );
		...
		spShaderInfo->m_Pass1.m_SpecularLocation = glGetUniformLocation(spShaderInfo->m_Pass1.m_ProgramObject, "Specular");
		spShaderInfo->m_Pass1.m_PowerLocation = glGetUniformLocation(spShaderInfo->m_Pass1.m_ProgramObject, "Power");
		...
		spShaderInfo->m_Pass1.m_LightSpecularLocation = glGetUniformLocation(spShaderInfo->m_Pass1.m_ProgramObject, "LightSpecular");
...
void CBlinnPhongShaderObject::Pass1Draw()
{
	...
	m_spLight->GetDirectionalLightData().m_SpecularColor.Copyf32Array3RGB( LightSpecular );
	glUniform3fv( m_spShaderInfo->m_Pass1.m_LightAmbientLocation , 1 , LightAmbient );
	glUniform3fv( m_spShaderInfo->m_Pass1.m_LightSpecularLocation , 1 , LightSpecular );
	...
	FOREACH( CSceneGraphNodePtrList , itMat , pMaterialContena->GetMaterialDataArray() )
	{
		...
		spMatDrawData->GetMaterial()->Specular.Copyf32Array3RGB( Specular );
		glUniform1fv( m_spShaderInfo->m_Pass1.m_AlphaLocation , 1 , &spMatDrawData->GetMaterial()->Alpha );
		glUniform1fv( m_spShaderInfo->m_Pass1.m_PowerLocation , 1 , &spMatDrawData->GetMaterial()->Power );
		glUniform3fv( m_spShaderInfo->m_Pass1.m_DiffuseLocation , 1 , Diffuse );
		glUniform3fv( m_spShaderInfo->m_Pass1.m_SpecularLocation , 1 , Specular );
