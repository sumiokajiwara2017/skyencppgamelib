...
SkyImplementRTTI( sky::lib::graphic::CTextureMappingShaderObject       , sky::lib::graphic::CShaderObject );
...
const serialize::CSerializeInterfacePtr& CTextureMappingShaderObject::Copy( const serialize::CSerializeInterfacePtr& spContainer )
{
	this->CSceneGraphNode::Copy( spContainer );
	return spContainer;
}

const serialize::CSerializeInterfacePtr& CTextureMappingShaderObject::CopySharing( const serialize::CSerializeInterfacePtr& spContainer )
{
	return Copy( spContainer );
}

serialize::CSerializeInterfacePtr CTextureMappingShaderObject::Clone()
{
	return Copy( CTextureMappingShaderObjectPtr( NEW__( CTextureMappingShaderObject , CTextureMappingShaderObject() ) ) );
}

serialize::CSerializeInterfacePtr CTextureMappingShaderObject::CloneSharing()
{
	return Clone();
}

CTextureMappingShaderObject::CTextureMappingShaderObject()
{
	m_PassNum = 1;
}

void CTextureMappingShaderObject::vDraw()
{
...
//頂点シェーダー入力値構造体
struct TextureMappingShaderConst
{
	//頂点変換行列
...
static CShaderInfoPtr GetShaderInfo( u32 shaderId )
{
	...
	if ( s_pShaderHash->Get( shaderId , spShaderInfo ) == skyFalse )
	{
		...
		// 頂点シェーダ作成
		HRESULT hr = D3DX11CompileFromFile( _T( "data/lib/shader/hlsl/TextureMapping.vert" ) , NULL , NULL , "mainVS" , CDevice_->GetMember()->GetVSVersion() , SHADER_COMPILE_OPTION , 0 , NULL , &pShader , &pError , NULL );
		if ( FAILED( hr ) )
		{
		...
		ConstDesc.ByteWidth			  = sizeof( TextureMappingShaderConst );
		...
		// ピクセルシェーダ作成
		hr = D3DX11CompileFromFile( _T( "data/lib/shader/hlsl/TextureMapping.frag" ) , NULL , NULL , "mainPS" , CDevice_->GetMember()->GetPSVersion() , SHADER_COMPILE_OPTION , 0 , NULL , &pShader , &pError , NULL );
		if ( FAILED( hr ) )
		{
			OutputDebugStringA( (const char*)pError->GetBufferPointer() ); pError->Release();
			D3DUtil::PrintD3D11Result( hr );
...
void CTextureMappingShaderObject::Pass1Draw()
{
	//定数バッファ受け渡し用の変数
	TextureMappingShaderConst shaderConst;
...
static CShaderInfoPtr GetShaderInfo( u32 shaderId )
{
	...
	if ( s_pShaderHash->Get( shaderId , spShaderInfo ) == skyFalse )
	{
		...
		sky::lib::graphic::CGLSL::CreateVertexShaderAndFragmentShader(
			hm_ProgramObject0 ,
			_T( "data/lib/shader/glsl/TextureMapping.vert" ), 
			_T( "data/lib/shader/glsl/TextureMapping.frag" ) );
...
static CShaderInfoPtr GetShaderInfo( u32 shaderId )
{
	...
	if ( s_pShaderHash->Get( shaderId , spShaderInfo ) == skyFalse )
	{
		...
		CCg::CreateVertexFragmentCombineShaderFromFile( spShaderInfo->m_Pass1.m_ProgramObject , 
		                                            "data/lib/shader/cg/TextureMapping.vert" , "mainVS" ,
		                                            "data/lib/shader/cg/TextureMapping.frag" , "mainPS"  );
...
void CTextureMappingShaderObject::Pass1Draw()
{
	...
	FOREACH( CSceneGraphNodePtrList , itMat , pMaterialContena->GetMaterialDataArray() )
	{
		...
		spVertexBuffer->GetUvStream()->GetMember()->m_Location   = 2;
		CVertexBufferStreamPtrList streamList;
		streamList.AddTail( spVertexBuffer->GetPositionStream() );
		streamList.AddTail( spVertexBuffer->GetColorStream() );
		streamList.AddTail( spVertexBuffer->GetUvStream() );
