...
using namespace cbmp24bitcolor;

void CBmp24BitColor::Clear( u32 abgr )
{
	DrawRect( m_Rect, abgr );
}

u32 CBmp24BitColor::GetDataSize()
{
	return m_pBmpInfo->bmiHeader.biWidth * m_pBmpInfo->bmiHeader.biHeight * cbmp24bitcolor::ONE_PIXEL_BYTE_SIZE;
}

void CBmp24BitColor::DrawPoint( const math::CBasicVector2 &pos , u32 abgr )
{
...
void CBmp24BitColor::DrawPoint( const math::CBasicVector2 &pos , u32 palletNo, u8 boldSize )
{
	//マイナス座標の場合リターン
...
void CBmp24BitColor::DrawLine( const math::CBasicVector2& start, math::CBasicVector2& Gole , u32 abgr, u8 boldSize )
{
	s32 offsetX ,offsetY,distNum ,drawBasePosX ,drawBasePosY;
	u32 twoPointDist;
	dectype	test1,test2,testX ,testY;
	...
	{
		test1 = ( ( dectype )offsetX/( dectype )distNum);
		test2 = ( ( dectype )offsetY/( dectype )distNum);
...
void CBmp24BitColor::DrawTile( CBmp24BitColor &bmpFile , math::CBasicVector2 &point, u32 nocpyColor )
{
...
void CBmp24BitColor::DrawRect( const math::CBasicRect& rect, u32 abgr )
{
...
void CBmp24BitColor::DrawTriangle( const math::CBasicTriangle3& tri, u32 abgr )
{
	abgr;
	tri;
}

void CBmp24BitColor::DrawCircle( const math::CBasicCircle& circle , u32 abgr )
{
...
u32 CBmp24BitColor::GetPoint( const math::CBasicVector2 &pos )
{
	//点内外判定
	if ( m_Rect.DotHitCheck( math::CBasicVector2( pos.X , pos.Y ) ) == skyFalse )
	{
		SKY_PANIC_MSG( _T( "The range of the bit map was exceeded." ) ); //ビットマップの範囲をこえました。
	}
...
void CBmp24BitColor::vLoad( stream::IStreamPtr &src )
{
	//リセット
	Reset();
	...
	//幅はアライメント取る
	m_LineWidth = PAGE_ALIGN( m_Rect.GetSize().W, cbmp24bitcolor::LINE_ALIGN_SIZE );
}

void CBmp24BitColor::vSave( stream::IStreamPtr &dest )
{
	//すべてストリームに書きだす
	dest->vWriteStream( m_pData , m_nDataSize );
}

void CBmp24BitColor::vCreate( u32 width, u32 height , u32 abgr )
{
	//幅はアライメント取る
	m_LineWidth = PAGE_ALIGN( width, cbmp24bitcolor::LINE_ALIGN_SIZE );

	u32 dwSizeImage = ( m_LineWidth * height * cbmp24bitcolor::ONE_PIXEL_BYTE_SIZE );
	...
	m_pBmpInfo->bmiHeader.biBitCount	=  cbmp24bitcolor::ONE_PIXEL_BIT_SIZE;				//1ピクセルあたりの情報量4bitつまり1バイトにつき2ピクセル
...
void CBmp24BitColor::vWriteFontInit( const CFontBmpInfo &info )
{
	//背景色でクリア（インデックスカラーはパレット０が背景色）
	Clear( info.in_clearcolor.GetAsABGR() );
}

void CBmp24BitColor::vWriteFont( const math::CBasicVector2 &pos , const CFontBmpInfo &info )
{
