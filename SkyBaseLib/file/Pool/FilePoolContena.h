...
/// プールファイルコンテナ
/// ファイルをメモリ上に保持しておき、
/// パスの問い合わせに対してファイルが存在していたらデータを返し、
/// 存在していなかったら存在していない旨を返す
/// </summary>
class CFilePoolContena : public base::SkyObject
{
...
private:
	...
	/// メンバ情報
	/// </summary>
	static CFilePoolContenaMember *m_pMember;

};

//インスタンスアクセス文字列
#define CFilePoolContenaInitialize_() sky::lib::file::CFilePoolContena::Initialize()
#define CFilePoolContena_             sky::lib::file::CFilePoolContena
#define CFilePoolContenaDispose_()    sky::lib::file::CFilePoolContena::Dispose()
